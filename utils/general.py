...
def plot_hand_cv2(image, coords_hw, linewidth=1):
    """ Plots a hand stick figure into a matplotlib figure. """
    colors = np.array([[0., 0., 0.5],
                       [0., 0., 0.73172906],
                       [0., 0., 0.96345811],
    ...
    for connection, color in bones:
        coord1 = coords_hw[connection[0], :]
        coord1_int = (int(coord1[1]), int(coord1[0]))

        coord2 = coords_hw[connection[1], :]
        coord2_int = (int(coord2[1]), int(coord2[0]))

        color_int = (color[0] * 255, color[1] * 255, color[2] * 255)

        cv2.line(image, coord1_int, coord2_int, color_int, linewidth)
